...
export default function ProductCard({ product }: { product: any }) {
  ...
  return (
    <Link href={`/products/${product.slug}`} className="group bg-white rounded-lg shadow-card hover:shadow-lg overflow-hidden border border-gray-100 transition-all hover:-translate-y-1">
      <div className="relative aspect-square overflow-hidden bg-gray-100">
        ...
        {product.isNew && <span className="absolute top-2 left-2 bg-green-500 text-white text-[10px] font-bold px-2 py-1 rounded">ใหม่</span>}
        {discount > 0 && <span className="absolute top-2 right-2 bg-brand-600 text-white text-[10px] font-bold px-2 py-1 rounded">-{discount}%</span>}
        {product.stock === 0 && <div className="absolute inset-0 bg-black/40 flex items-center justify-center text-white font-bold">หมด</div>}
      </div>
      <div className="p-3">
        <div className="text-xs text-gray-500 mb-1">{product.brand || ''}</div>
        <h3 className="text-sm font-medium line-clamp-2 min-h-[40px] group-hover:text-brand-600">{product.name}</h3>
